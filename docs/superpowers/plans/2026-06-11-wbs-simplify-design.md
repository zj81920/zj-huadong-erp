...
- [x] **Step 1: 在测试文件末尾添加 `computeTaskStatus` 的失败测试**
...
- [x] **Step 3: 实现 `computeTaskStatus()` 函数**
...
- [x] **Step 3: 实现 `computeParentStatus()`**
...
- [x] **Step 1: 重写进度弹窗（合并编辑：任务名 + 日期 + 进度）**
...
- [x] **Step 2: 重写状态计算逻辑（L4 用 computeTaskStatus，L1-L3 用 computeParentStatus）**
...
- [x] **Step 5: 重写责任人列（responsibleIds JSON 数组）**
...
- [x] **Step 3: 重写甘特图行渲染（单条 progress 横道 + 计划虚线框）**
...
以下功能在新状态系统稳定后实现，不放入本轮计划：

- **AI 生成 L4 任务**: `/api/projects/plans/[projectSourceId]/nodes/generate-tasks` — LLM 提示词注入石油化工领域知识
- **AI 报告生成**: `/api/projects/plans/[projectSourceId]/report` — 调用 LLM 生成结构化报告
- **责任人 UI**: `ResponsibleSelect` 组件适配 `responsibleIds` JSON 数组，支持多选+搜索
...
```
Task 0 (computeTaskStatus TDD) ──┐
Task 1 (computeParentStatus TDD) ─┤─ 必须先于所有前端 Task
Task 2 (aggregateProgress TDD) ──┘
        │
Task 3  (Prisma Schema) ── 必须先于 Task 4/5/6
        │
Task 4  (Progress API) ──┐
Task 5  (Phase Sync Fix) ─┤─ 可并行
Task 6  (Summary API) ────┘
        │
Task 7  (Dashboard) ──┐
Task 8  (AlertBar) ────┤─ 可并行
Task 9  (ProgressDialog)┘
        │
Task 10 (WbsTreeList) ──┐─ 可并行
Task 11 (GanttChart) ───┘
        │
Task 12 (Verify)
```

**Summary:** 13 个 Task，50+ 个 Step，预计逐 Task 提交，TDD 覆盖核心纯函数。