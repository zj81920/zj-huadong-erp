...
| 列 | 宽度 | 说明 |
|----|------|------|
| 项目编号 | 100px | projectCode / sourceRefId |
| 项目名称 | 200px | 过长省略号 |
| 设计阶段 | 自适应 | 阶段标签（含采购），蓝色小标签展示 |
| 甲方 | 100px | 客户名称 |
| 进度 | 110px | 进度条 + 百分比，颜色反映状态 |
| 状态 | 70px | emoji+文字 badge，延误行显示「N项延误」子信息 |
| 风险 | 40px | 🟢低 / 🟡中 / 🔴高 圆点 |

**风险等级**: 0 延误=🟢低风险，1-2 延误=🟡中风险，≥3 延误=🔴高风险

**视觉**: 延误项目行浅红背景突出

**Summary API 变更**: `judgeTaskStatus` → `computeTaskStatus`，去掉 actualStart/End 依赖
