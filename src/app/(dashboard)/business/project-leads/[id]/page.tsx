...
export default function ProjectLeadDetailPage() {
  ...
  if (loading) return <div className="empty-state min-h-[60vh]"><div className="w-10 h-10 border-2 border-[#007AFF] border-t-transparent rounded-full animate-spin" /><p>加载中...</p></div>;
  if (!lead) return <div className="empty-state min-h-[60vh]"><Briefcase className="w-8 h-8 text-[#86868B]" /><p>项目线索不存在</p><button className="ios-btn ios-btn-primary mt-4" onClick={() => router.push("/business/project-leads")}>返回列表</button></div>;
    ...
    <>
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-[20px] font-bold text-[#1D1D1F] leading-tight">{lead.projectName}</h1>
                <p className="text-[13px] text-[#007AFF] font-mono font-semibold">{lead.projectSourceId}</p>
              </div>
              <span className={`ios-badge ml-2 ${sc.color}`}>{sc.label}</span>
            </div>
            {!isEstablished && (
              <div className="flex items-center gap-0.5 bg-[#F5F5F7] rounded-lg p-0.5 mt-2">
                <button
                  className={`px-3 py-1 rounded-md text-[12px] font-medium transition-all ${lead.leadMode !== "商务报价" ? "bg-white text-[#1D1D1F] shadow-sm" : "text-[#86868B]"}`}
                  onClick={async () => {
                    try {
                      const res = await fetch(`/api/project-leads/${lead.id}`, {
                        method: "PUT",
      ...
      <div className="mb-8">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="text-[15px] font-bold text-[#1D1D1F]">基本信息</h2>
          {!isEstablished && (
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="bento-card-static"><p className="text-[12px] text-[#86868B] mb-1">客户</p><p className="text-[15px] font-semibold text-[#1D1D1F]">{lead.customer.name}</p>{lead.customer.industryType && <span className={`ios-badge text-[10px] mt-1 ${lead.customer.industryType === "石化" ? "ios-badge-orange" : "ios-badge-green"}`}>{lead.customer.industryType}</span>}</div>
          <div className="bento-card-static"><p className="text-[12px] text-[#86868B] mb-1">项目地点</p><p className="text-[15px] font-semibold text-[#1D1D1F]">{lead.location || "-"}</p></div>
          <div className="bento-card-static"><p className="text-[12px] text-[#86868B] mb-1">项目联系人</p><p className="text-[15px] font-semibold text-[#1D1D1F]">{lead.contactPerson || "-"}</p></div>
          <div className="bento-card-static"><p className="text-[12px] text-[#86868B] mb-1">联系电话</p><p className="text-[15px] font-semibold text-[#1D1D1F]">{lead.contactPhone || "-"}</p></div>
          <div className="bento-card-static"><p className="text-[12px] text-[#86868B] mb-1">联系邮箱</p><p className="text-[15px] font-semibold text-[#1D1D1F]">{lead.contactEmail || "-"}</p></div>
          <div className="bento-card-static"><p className="text-[12px] text-[#86868B] mb-1">项目性质</p><div className="flex flex-wrap gap-1 mt-1">{(lead.projectNature || []).map((n: string) => <span key={n} className="ios-badge text-[10px] ios-badge-blue">{n}</span>)}</div></div>
          <div className="bento-card-static"><p className="text-[12px] text-[#86868B] mb-1">实施主体</p><p className="text-[15px] font-semibold text-[#1D1D1F]">{lead.implementationEntity || "-"}</p></div>
          <div className="bento-card-static"><p className="text-[12px] text-[#86868B] mb-1">创建时间</p><p className="text-[15px] font-semibold text-[#1D1D1F]">{formatDate(lead.createdAt)}</p></div>
        </div>
      </div>
      ...
      <Modal isOpen={showLeadEditModal} onClose={() => setShowLeadEditModal(false)} title="编辑项目线索" maxWidth="640px">
        <div className="space-y-4">
          {leadError && <div className="p-3 rounded-xl bg-[#FF3B30]/8 text-[#FF3B30] text-[13px] font-medium">{leadError}</div>}
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-[13px] font-semibold text-[#1D1D1F] mb-1.5">客户 <span className="text-[#FF3B30]">*</span></label>
              ...
              <button type="button" className="ios-btn ios-btn-ghost ios-btn-sm text-[#007AFF] mt-1" onClick={() => { setCustomerError(""); setShowCustomerModal(true); }}>
                <Plus className="w-3.5 h-3.5" />新增客户
              </button>
            </div>
            <div>
              <label className="block text-[13px] font-semibold text-[#1D1D1F] mb-1.5">项目名称 <span className="text-[#FF3B30]">*</span></label>
              <input type="text" className="ios-input" value={leadForm.projectName} onChange={(e) => setLeadForm((p) => ({ ...p, projectName: e.target.value }))} />
            </div>
            <div><label className="block text-[13px] font-semibold text-[#1D1D1F] mb-1.5">项目地点</label><input type="text" className="ios-input" value={leadForm.location} onChange={(e) => setLeadForm((p) => ({ ...p, location: e.target.value }))} /></div>
            <div><label className="block text-[13px] font-semibold text-[#1D1D1F] mb-1.5">项目联系人</label><input type="text" className="ios-input" placeholder="请输入联系人" value={leadForm.contactPerson} onChange={(e) => setLeadForm((p) => ({ ...p, contactPerson: e.target.value }))} /></div>
            <div><label className="block text-[13px] font-semibold text-[#1D1D1F] mb-1.5">联系电话</label><input type="text" className="ios-input" placeholder="请输入联系电话" value={leadForm.contactPhone} onChange={(e) => setLeadForm((p) => ({ ...p, contactPhone: e.target.value }))} /></div>
            <div><label className="block text-[13px] font-semibold text-[#1D1D1F] mb-1.5">联系邮箱</label><input type="email" className="ios-input" placeholder="请输入联系邮箱" value={leadForm.contactEmail} onChange={(e) => setLeadForm((p) => ({ ...p, contactEmail: e.target.value }))} /></div>
            <div>
              <label className="block text-[13px] font-semibold text-[#1D1D1F] mb-1.5">项目性质 <span className="text-[#FF3B30]">*</span></label>
                  ...
                  return (
                    <button
                      ...
                      className={`px-2.5 py-1 rounded-lg text-[12px] font-medium transition-all ${
                        selected ? "bg-[#007AFF] text-white" : "bg-[#F5F5F7] text-[#86868B] hover:bg-[#E8E8ED]"
                      }`}
                      onClick={() => {
            ...
            <div>
              <label className="block text-[13px] font-semibold text-[#1D1D1F] mb-1.5">实施主体 <span className="text-[#FF3B30]">*</span></label>
      ...
      <Modal isOpen={showCustomerModal} onClose={() => setShowCustomerModal(false)} title="新增客户" maxWidth="600px">
        <div className="space-y-4">
          {customerError && <div className="p-3 rounded-xl bg-[#FF3B30]/8 text-[#FF3B30] text-[13px] font-medium">{customerError}</div>}
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2">
              <label className="block text-[13px] font-semibold text-[#1D1D1F] mb-1.5">客户名称 <span className="text-[#FF3B30]">*</span></label>
