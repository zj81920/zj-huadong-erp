...
import { ExpenseContractDetailCard } from '@/components/detail-cards';
...
export default function ExpenseContractsPage() {
  ...
  const hasFlow = user?.moduleFlowStatus?.["expense_contract"] ?? false;
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload = {
        ...
        reviewResult: form.reviewResult || null,
  ...
  return (
    <>
      ...
      <div className="bento-card-static">
                  ...
                  <tr key={contract.id} className={`${isSelected(contract.id) ? "bg-[#1C1917]/5" : ""} ${getRowStatusClass(contract.status)}`}>
                    ...
                    <td>
                      {contract.project ? (
                        ...
                        <span className="text-[#78716C]">{contract.projectSourceId}</span>
                      ) : (
                    ...
                    <td>
                      <div className="flex items-center gap-1">
                            ...
                            <button
                              ...
                              onClick={() => {
                                ...
                                setInvoiceForm({
                                  ...
                                  taxAmount: "",
                                  totalAmount: "",
                                  sellerName: contract.supplier?.name || "",
                                  sellerTaxNo: "",
                                  remark: "",
                                  attachments: [],
                                });
                        ...
                        {contract.status === "合同归档" && (
                          <button
                            className="ios-btn ios-btn-ghost ios-btn-sm"
                            onClick={() => router.push(`/contracts/change-orders/new?contractType=expense_contract&contractId=${contract.id}`)}
                          >
                            发起变更
                          </button>
                        )}
        ...
        {rolePerms.delete && <BatchDeleteBar businessType="expense_contract" selectedIds={contracts.filter(d => isSelected(d.id)).map(d => d.id)} onDeleteSuccess={fetchContracts} onClear={clearSelection} />}
      </div>
      ...
      >
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-[13px] font-semibold text-[#1C1917] mb-1.5">
              付款条款 {aiAnalyzing && <span className="text-[#78716C] font-normal">（AI生成中...）</span>}
            </label>
            <textarea
      ...
      >
        {detailLoading ? (
          ...
          >
            ...
            <ExpenseContractDetailCard data={detailContract} />

            {detailContract.scannedUrl && (
              <div>
                <p className="text-[12px] text-[#78716C] mb-0.5">扫描件</p>
                <a
                  href={detailContract.scannedUrl}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-[14px] text-[#1C1917] hover:underline"
                >
                  {detailContract.scannedUrl}
                </a>
              </div>
            ...
            {detailContract.inquiry && (
              <div className="p-3 rounded-xl bg-[#FAFAF9]">
                <p className="text-[12px] text-[#78716C] mb-2">关联询价单</p>
                <div className="text-[13px] mb-2">
                  <span className="text-[#78716C]">采购需求：</span>
                  ...
                  <>
                    ...
                    <div className="mt-3 p-3 rounded-xl bg-[#FAFAF9]">
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-[12px] text-[#78716C]">已收票金额 / 合同总额</span>
            ...
            {detailContract.deliveryReceipts && detailContract.deliveryReceipts.length > 0 && (
              <div>
                <div className="flex items-center gap-2 mb-2">
                  <FileCheck className="w-4 h-4 text-[#78716C]" />
                  <p className="text-[13px] font-semibold text-[#1C1917]">
                ...
                <div className="space-y-2">
                  {detailContract.deliveryReceipts.map((receipt) => (
                    <div key={receipt.id} className="flex items-center justify-between p-2.5 rounded-xl bg-[#FAFAF9]">
                      <div className="text-[13px]">
                        <span className="text-[#1C1917] font-medium">{receipt.receivedQuantity}</span>
                        {receipt.deliveryAmount && <span className="text-[#1C1917] ml-2 font-semibold">¥{parseFloat(receipt.deliveryAmount).toLocaleString("zh-CN", { minimumFractionDigits: 2 })}</span>}
                        <span className="text-[#78716C] ml-2">{new Date(receipt.deliveryDate).toLocaleDateString("zh-CN")}</span>
                      </div>
                      <div className="flex items-center gap-2">
            ...
            {detailContract.contractType === "项目采购" && (
              <div className="grid grid-cols-3 gap-3 p-3 rounded-xl bg-[#FAFAF9]">
                <div>
                  <p className="text-[12px] text-[#78716C] mb-0.5">合同金额</p>
                  <p className="text-[14px] font-bold text-[#1C1917]">{formatAmount(detailContract.totalAmount)}</p>
                </div>
                <div>
                  <p className="text-[12px] text-[#78716C] mb-0.5">已结算金额</p>
                  <p className="text-[14px] font-bold text-[#1C1917]">{formatAmount(detailContract.settledAmount || "0")}</p>
                </div>
                <div>
                  <p className="text-[12px] text-[#78716C] mb-0.5">结算状态</p>
      ...
      >
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-[13px] font-semibold text-[#1C1917] mb-1.5">销方名称</label>
              <input
                type="text"
                className="ios-input"
            ...
            <div>
              <label className="block text-[13px] font-semibold text-[#1C1917] mb-1.5">销方税号</label>
              <input
                type="text"
                className="ios-input"
