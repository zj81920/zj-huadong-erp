...
export default function NonContractPage() {
  ...
  const formatAmount = (amount: number, type: TabType) => {
    ...
    return type === "income" ? (
      <span className="text-[#78716C] font-semibold">{formatted}</span>
    ) : (
      <span className="text-[#78716C] font-semibold">{formatted}</span>
    );
  };
    ...
    <>
                ...
                {records.map((record) => {
                  ...
                  return (
                    <tr key={record.id} className={`${isSelected(record.id) ? "bg-[#1C1917]/5" : ""} ${getRowStatusClass(record.status)}`}>
                      ...
                      <td className="text-[#78716C]">{formatDate(record.transactionDate)}</td>
                      <td>
                        {record.projectSourceId ? (
                          <div>
                            ...
                            {record.project && (
                              <p className="text-[11px] text-[#78716C] mt-0.5">{record.project.name}</p>
                            )}
                          </div>
                        ) : (
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-[13px] font-semibold text-[#1C1917] mb-1.5">
                交易对方 <span className="text-[#78716C]">*</span>
              </label>
              <input
                type="text"
                className="ios-input"
            ...
            <div>
              <label className="block text-[13px] font-semibold text-[#1C1917] mb-1.5">
                金额（元） <span className="text-[#78716C]">*</span>
              </label>
              <input
                type="number"
                className="ios-input"
                ...
                {expenseInvoices.length > 0 ? (
                  <>
                    ...
                    <div className="mt-3 p-3 rounded-xl bg-[#FAFAF9]">
                      <div className="flex items-center justify-between">
                        <span className="text-[12px] text-[#78716C]">已收票金额 / 支出金额</span>
                      ...
                      <div className="w-full h-2 rounded-full bg-[#E7E5E4] overflow-hidden mt-2">
                        <div
      ...
      >
        <div className="text-center">
          ...
          <p className="text-[15px] text-[#1C1917] mb-1">确定要删除该{tabLabel}记录吗？</p>
