...
const progressColor = (value: number) => {
  if (value >= 80) return "bg-[#34C759]";
  if (value >= 50) return "bg-[#007AFF]";
  if (value >= 20) return "bg-[#FF9500]";
  return "bg-[#FF3B30]";
};
    ...
    <>
      ...
      <div className="grid grid-cols-3 gap-5 mb-6">
        <div className="bento-card-static flex items-center gap-4">
          ...
          <div>
            <p className="text-[13px] text-[#86868B]">计划总数</p>
        ...
        <div className="bento-card-static flex items-center gap-4">
          ...
          <div>
            <p className="text-[13px] text-[#86868B]">进行中</p>
            <p className="text-[24px] font-bold text-[#34C759] leading-tight">{stats.inProgress}</p>
          </div>
        </div>
        <div className="bento-card-static flex items-center gap-4">
          ...
          <div>
            <p className="text-[13px] text-[#86868B]">已完成</p>
            <p className="text-[24px] font-bold text-[#FF9500] leading-tight">{stats.completed}</p>
          </div>
        </div>
      </div>
      ...
      <div className="bento-card-static">
        <div className="filter-bar">
          ...
          <div className="ml-auto text-[13px] text-[#86868B]">
            共 <span className="font-semibold text-[#1D1D1F]">{pagination.total}</span> 条计划
          </div>
        </div>
                ...
                {plans.map((plan) => {
                  ...
                  return (
                    <tr key={plan.id}>
                      ...
                      <td className="text-[#86868B]">{formatDate(plan.startDate)}</td>
                      <td className="text-[#86868B]">{formatDate(plan.endDate)}</td>
                      ...
                      <td>
                        <div className="flex items-center gap-2 min-w-[80px]">
                          ...
                          <span className="text-[12px] text-[#86868B] w-8 text-right">{plan.actualProgress}%</span>
                        </div>
                      </td>
                      <td>
      ...
      >
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-[13px] font-semibold text-[#1D1D1F] mb-1.5">
                计划类型 <span className="text-[#FF3B30]">*</span>
              </label>
              <select
                className="ios-select"
                value={form.planType}
            ...
            <div className="col-span-2">
              <label className="block text-[13px] font-semibold text-[#1D1D1F] mb-1.5">
                计划内容 <span className="text-[#FF3B30]">*</span>
              </label>
              <textarea
            ...
            <div>
              <label className="block text-[13px] font-semibold text-[#1D1D1F] mb-1.5">
                开始日期 <span className="text-[#FF3B30]">*</span>
              </label>
            ...
            <div>
              <label className="block text-[13px] font-semibold text-[#1D1D1F] mb-1.5">
                结束日期 <span className="text-[#FF3B30]">*</span>
              </label>
              <div className="relative">
                <Calendar className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-[#86868B]" />
                <input
            ...
            {editingPlan && (
              <div>
                <label className="block text-[13px] font-semibold text-[#1D1D1F] mb-1.5">进度 (%)</label>
                <input
                  type="number"
                  className="ios-input"
                  min="0"
        ...
        {detailPlan && (
          <div className="space-y-5">
            <div className="flex items-center gap-3 pb-4 border-b border-[#F0F0F0]">
              ...
              <div>
                <p className="text-[17px] font-bold text-[#1D1D1F]">{detailPlan.project?.name || "-"}</p>
                <p className="text-[13px] text-[#007AFF] font-mono font-semibold">{detailPlan.projectSourceId}</p>
              </div>
            ...
            <div>
              <p className="text-[12px] text-[#86868B] mb-1">计划内容</p>
              <p className="text-[14px] text-[#1D1D1F] leading-relaxed">{detailPlan.planContent}</p>
            </div>

            <div className="grid grid-cols-2 gap-4">
              <div className="p-3 rounded-xl bg-[#F5F5F7]">
                <p className="text-[12px] text-[#86868B] mb-1">开始日期</p>
                <p className="text-[14px] font-semibold text-[#1D1D1F]">{formatDate(detailPlan.startDate)}</p>
              </div>
              <div className="p-3 rounded-xl bg-[#F5F5F7]">
                <p className="text-[12px] text-[#86868B] mb-1">结束日期</p>
                <p className="text-[14px] font-semibold text-[#1D1D1F]">{formatDate(detailPlan.endDate)}</p>
              </div>
              <div className="p-3 rounded-xl bg-[#F5F5F7]">
                <p className="text-[12px] text-[#86868B] mb-1">负责人</p>
                <p className="text-[14px] font-semibold text-[#1D1D1F]">{detailPlan.responsiblePerson?.realName || "-"}</p>
              </div>
              <div className="p-3 rounded-xl bg-[#F5F5F7]">
                <p className="text-[12px] text-[#86868B] mb-1">版本</p>
                <p className="text-[14px] font-semibold text-[#1D1D1F]">v{detailPlan.version}</p>
              </div>
              <div className="p-3 rounded-xl bg-[#F5F5F7]">
                <p className="text-[12px] text-[#86868B] mb-1">创建时间</p>
                <p className="text-[14px] font-semibold text-[#1D1D1F]">{formatDate(detailPlan.createdAt)}</p>
              </div>
              <div className="p-3 rounded-xl bg-[#F5F5F7]">
                <p className="text-[12px] text-[#86868B] mb-1">更新时间</p>
                <p className="text-[14px] font-semibold text-[#1D1D1F]">{formatDate(detailPlan.updatedAt)}</p>
              </div>
            </div>

            <div className="p-4 rounded-xl bg-[#F5F5F7]">
              <div className="flex items-center justify-between mb-2">
                <p className="text-[12px] text-[#86868B]">实际进度</p>
                <p className="text-[14px] font-bold text-[#1D1D1F]">{detailPlan.actualProgress}%</p>
              </div>
              <div className="h-3 rounded-full bg-[#E5E5EA] overflow-hidden">
                <div
